...
from engine.macro_swarm import SwarmConfig, swarm_manager, MacroSwarm, AgentRole
from engine.state import AgentStatus
from engine.safeguards import SafeguardSystem, AlertSeverity
...
class SimulationRunner:
    ...
    def __init__(
        ...
        self.swarm: Optional[MacroSwarm] = None
        self.safeguard: Optional[SafeguardSystem] = None
    ...
    def run_to_completion(self) -> Dict[str, Any]:
        ...
        for tick_num in range(1, self.max_ticks + 1):
            ...
            if result["global_loss"] < self.convergence_threshold:
                self._log(f"Converged at tick {tick_num} with loss {result['global_loss']:.6f}")
                converged = True
                break
...
def run_sync_simulation(
    ...
    return asyncio.run(run_async_simulation(
    num_agents=num_agents,
    max_ticks=max_ticks,
    global_objective=global_objective,
    loss_function=loss_function,
...
def main():
    ...
    parser.add_argument(
    "--objective", "-o", type=str, default="minimize_loss",
    ...
    parser.add_argument(
    "--checkpoint", "-c", type=int, default=10,
