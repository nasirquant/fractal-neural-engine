...
from .state import (
    AgentState, AgentMessage, SimulationTickPacket, AgentRole, AgentStatus,
    MessageType, CompressedVector, EpochResult
)
from .graph_rag import graph_rag_manager, GraphRAG
...
class SwarmConfig(BaseModel):
    ...
    agent_roles: List[AgentRole] = Field(default_factory=lambda: [
        AgentRole.EXPLORER, AgentRole.OPTIMIZER, AgentRole.CRITIC,
...
class MacroSwarm:
    ...
    def build_agent_graph(self, agent_node: AgentNode) -> StateGraph:
        ...
        def think_node(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return {"agent_id": agent.agent_id, "thought": "completed"}
        ...
        def act_node(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            return {"agent_id": agent.agent_id, "action": "completed"}
    ...
    def _get_agent_context(self, agent_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "global_loss": self.global_loss_history[-1] if self.global_loss_history else 0.0,
            "tick": self.tick_number,
        }
    
    def _simulate_thinking(self, agent: AgentNode, context: Dict[str, Any]) -> CompressedVector:
    ...
    def _mse_loss(self, contributions: Dict[str, float]) -> float:
        ...
        if not contributions:
            return 1.0
        target = 1.0
        mse = sum((target - c) ** 2 for c in contributions.values()) / len(contributions)
        return min(1.0, mse)
    ...
    def _cosine_loss(self, contributions: Dict[str, float]) -> float:
        ...
        target = np.ones_like(values)
        if np.linalg.norm(values) == 0:
            return 1.0
        cos_sim = np.dot(values, target) / (np.linalg.norm(values) * np.linalg.norm(target))
        return 1.0 - cos_sim
    ...
    def get_epoch_result(self) -> EpochResult:
        ...
        return EpochResult(
            ...
            converged=self.global_loss_history[-1] < self.config.convergence_threshold if self.global_loss_history else False,
            final_global_loss=self.global_loss_history[-1] if self.global_loss_history else 1.0,
            ...
            total_duration_seconds=(datetime.utcnow() - self._start_time).total_seconds(),
        )
...
class SwarmManager:
    ...
    def get_swarm(self, epoch_id: str) -> Optional[MacroSwarm]:
