...
from .state import AgentState, SimulationTickPacket, AgentStatus
...
class Checkpoint:
    ...
    tick_number: int = 0
    timestamp: datetime = field(default_factory=datetime.utcnow)
    tick_packet: Optional[SimulationTickPacket] = None
...
class DivergenceMonitor:
    ...
    def update_agent_score(self, agent_id: str, score: float) -> Optional[SafetyAlert]:
        ...
        with self._lock:
            ...
            if len(self._agent_scores[agent_id]) > self.window_size:
                self._agent_scores[agent_id] = self._agent_scores[agent_id][-self.window_size:]
            
            # Check for alert
            if score >= self.alert_threshold:
                alert = SafetyAlert(
                    severity=AlertSeverity.CRITICAL if score >= self.max_divergence_score else AlertSeverity.WARNING,
                    source="divergence_monitor",
                    message=f"Agent {agent_id} divergence score {score:.2f} exceeds threshold",
                    details={"agent_id": agent_id, "score": score, "threshold": self.alert_threshold}
                )
    ...
    def update_swarm_score(self, score: float) -> Optional[SafetyAlert]:
    ...
    def get_recent_alerts(self, since: Optional[datetime] = None, limit: int = 100) -> List[SafetyAlert]:
        """Get recent alerts."""
        with self._lock:
            alerts = self._alerts
            if since:
...
class CheckpointManager:
    ...
    def _delete_checkpoint(self, checkpoint_id: str) -> None:
        ...
        if checkpoint.epoch_id in self._epoch_checkpoints:
            self._epoch_checkpoints[checkpoint.epoch_id] = [
                cid for cid in self._epoch_checkpoints[checkpoint.epoch_id]
    ...
    def load_checkpoint(self, checkpoint_id: str) -> Optional[SimulationTickPacket]:
    ...
    def get_latest_checkpoint(self, epoch_id: str) -> Optional[Checkpoint]:
    ...
    def rollback_to_checkpoint(self, checkpoint_id: str) -> Optional[SimulationTickPacket]:
...
class SafeguardSystem:
    ...
    def __init__(
        ...
        self.max_divergence_score = max_divergence_score or settings.max_divergence_score
        self.checkpoint_interval = checkpoint_interval
        
        self.circuit_breakers = CircuitBreakerRegistry()
        self.divergence_monitor = DivergenceMonitor(max_divergence_score=self.max_divergence_score)
    ...
    def _register_breaker_callbacks(self) -> None:
        ...
        for name, breaker in self.circuit_breakers._breakers.items():
            breaker.add_state_change_callback(
                lambda old, new, n=name: self._on_breaker_state_change(n, old, new)
            )

    def _on_breaker_state_change(self, name: str, old_state: CircuitState, new_state: CircuitState) -> None:
        """Handle circuit breaker state changes."""
        if new_state == CircuitState.OPEN:
            self._circuit_break_count += 1
            logger.warning(f"Circuit breaker '{name}' opened (total breaks: {self._circuit_break_count})")
    ...
    def on_tick_end(
        ...
        with self._lock:
            ...
            swarm_alert = self.divergence_monitor.update_swarm_score(tick_packet.global_loss)
            if swarm_alert:
            ...
            if self.divergence_monitor.is_swarm_diverged():
                alerts.append(SafetyAlert(
                    ...
                    details={"swarm_score": self.divergence_monitor.get_swarm_score()}
                ))
            ...
            if self.divergence_monitor.is_swarm_diverged():
                ...
                if rollback_packet:
                    ...
                    alerts.append(SafetyAlert(
                        ...
                        details={"rollback_tick": rollback_packet.tick_number, "rollback_count": self._rollback_count}
                    ))
                ...
                if self.divergence_monitor.is_agent_diverged(agent_id):
                    ...
                    if rollback_packet:
                        ...
                        alerts.append(SafetyAlert(
                            ...
                            details={"agent_id": agent_id, "rollback_tick": rollback_packet.tick_number, "rollback_count": self._rollback_count}
                        ))
        ...
        with self._lock:
            return {
                ...
                "checkpoints": {
                    ...
                    "epoch_checkpoints": len(self.checkpoint_manager.get_epoch_checkpoints(self.epoch_id))
                },
                "alerts": {
    ...
    def emergency_stop(self) -> Optional[SimulationTickPacket]:
