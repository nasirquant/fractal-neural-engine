...
class SkillValidator:
    ...
    def validate_signature(
        self, source_code: str, expected_signature: str
    ) -> tuple[bool, list[str]]:
...
class SkillCompiler:
    ...
    def analyze_failures(
        self, trajectories: list[FailureTrajectory]
    ) -> list[dict[str, Any]]:
    ...
    def extract_workflow(
        self, epoch_id: str, tick_range: tuple[int, int], agent_id: str
    ) -> str | None:
...
'''
    ...
    def _compile_skill_internal(
        ...
        return CompilationResult(
            success=True, skill_id=skill_id, test_results=test_results
        )
        valid, sig_errors = self._validator.validate_signature(source_code, "execute")
        if not valid:
            return CompilationResult(success=False, error="; ".join(sig_errors))
    ...
    def _wrap_skill(self, source_code: str, skill_id: str) -> str:
        ...
        if "re.sub" in source_code:
            ...
            print(f"DEBUG _wrap_skill source_code snippet: {source_code[idx:idx+60]!r}")
        wrapper = f'''
"""
Skill: {skill_id}
...
'''
        if "re.sub" in wrapper:
            ...
            print(f"DEBUG _wrap_skill result snippet: {wrapper[idx:idx+60]!r}")
        return wrapper
    ...
    def _load_skill(self, skill_id: str, skill_file: Path) -> CompilationResult:
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                return CompilationResult(
                    success=False, error="Failed to create module spec"
                )
            ...
            if not hasattr(module, "safe_execute"):
                return CompilationResult(
                    success=False, error="No safe_execute function found"
                )
    ...
    def record_failure(self, trajectory: FailureTrajectory) -> None:
        ...
        with self._lock:
            ...
            if len(self._failure_trajectories) > max_trajectories:
                self._failure_trajectories = self._failure_trajectories[
                    -max_trajectories:
                ]
...
class SkillRegistry:
    ...
    def get_compiler(
        self, epoch_id: str, skills_dir: str | None = None
    ) -> SkillCompiler:
